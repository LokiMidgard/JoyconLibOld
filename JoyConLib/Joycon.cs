...
namespace JoyCon
{
    ...
    public class Joycon
    {
        ...
        {
            return Quartainion.LookAt(new Vector3(j_b.X, i_b.X, k_b.X), -(new Vector3(j_b.Z, i_b.Z, k_b.Z)));
        ...
        public void Update()
        {
            if (state > state_.NO_JOYCONS)
            {
                ...
                if (rumble_obj.timed_rumble)
                {
                    ...
                    {
                        rumble_obj.t -= 1f;//Time.deltaTime;
                    }
                }
            }
        }
        ...
        private void ExtractIMUValues(byte[] report_buf, int n = 0)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                Do(ref acc_g, i) = acc_r[i] * 0.00025f;
                Do(ref gyr_g, i) = (gyr_r[i] - gyr_neutral[i]) * 0.00122187695f;
                if (Math.Abs(Do(ref acc_g, i)) > Math.Abs(max[i]))
                    max[i] = Do(ref acc_g, i);
            }
